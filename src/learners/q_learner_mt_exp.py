...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.rew_mean_log = 0.
        self.bwm_log = np.zeros(self.args.scenario_len)
        self.alpha = self.args.alpha
        if self.args.use_anneal:
            self.anneal_delta = 1 / self.args.exp_anneal_time
        self.bwm_this_batch = np.zeros(self.args.scenario_len)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if 'imagine' in self.args.agent:
            ...
            rep_actions = actions.repeat(4, 1, 1, 1)
            all_chosen_action_qvals = th.gather(all_mac_out[:, :-1], dim=3, index=rep_actions).squeeze(3)  # Remove the last dim

            mac_out, moW, moI, moD = all_mac_out.chunk(4, dim=0)
            chosen_action_qvals, caqW, caqI, caqD = all_chosen_action_qvals.chunk(4, dim=0)
            ...
            if 'imagine' in self.args.agent:
                ...
                if will_log and self.args.test_gt_factors:
                    ...
                    caq_imagine = self.mixer(caq_imagine, mix_ins,
                                             imagine_groups=groups[:2])
                    caq_drop = self.mixer(caqD, mix_ins, imagine_groups=groups[-1])
            else:
                mix_ins, targ_mix_ins = self._get_mixer_ins(batch)
        ...
        if 'imagine' in self.args.agent:
            ...
            sce = th.nonzero(batch['scenario'][:, 0] == 1)[:, -1]
            bw_this_batch = 1 - th.tensor(self.bwm_log[sce.cpu()]).cuda()
            weight = F.softmax(bw_this_batch / self.args.tempr, dim=-1).float()

            im_td_error = caq_imagine - targets.detach()
            im_masked_td_error = im_td_error * mask
            im_loss = (im_masked_td_error ** 2).sum(1) / mask.sum(1)
            im_loss = (im_loss.permute(1, 0) * weight).sum()

            exp_td_error = caq_drop - targets.detach()
            exp_masked_td_error = exp_td_error * mask
            exp_loss = (exp_masked_td_error ** 2).sum(1) / mask.sum(1)
            exp_loss = (exp_loss.permute(1, 0) * weight).sum()

            if self.args.use_anneal:
                alpha = min(1., self.anneal_delta * t_env)
            else:
                alpha = self.alpha
            loss = (1 - im_prop) * loss + + im_prop * self.args.lam * (alpha * im_loss + (1 - alpha) * exp_loss)
        ...
        self.optimiser.step()
        self.bwm_this_batch = bw_this_batch
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if 'imagine' in self.args.agent:
                ...
                self.logger.log_stat("loss_o", loss_o.item(), t_env)
                self.logger.log_stat("bwm_mean", self.bwm_log.mean(), t_env)
                self.logger.log_stat("exp_loss", exp_loss.item(), t_env)
    ...
    def update_rew_log(self, rew):
        self.rew_mean_log = rew
    def update_bwm(self, bwm):
        self.bwm_log = bwm
