...
class StarCraft2CustomEnv(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.sce = None
        self.sce_eye = np.eye(len(self.scenarios))
    ...
    def init_units(self, unit_override=None, index=None):
        ...
        while True:
            ...
            if not self.max_reward_init:
                ...
                # idx_here = self.rs.choice(range(len(self.scenarios)), 1, p=self.sample_p)[0]
                # scenario = self.scenarios[idx_here]
                idx = self.rs.randint(len(self.scenarios))
