...
def run(_run, _config, _log):
    ...
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(args.local_results_path, args.tb_dirname)
...
def evaluate_sequential(args, runner, logger):
    ...
    for i in range(n_scen):
        ...
        if args.eval_all_scen:
            ...
            if 'sc2' in args.env:
                scen_str = "-".join("%i%s" % (count, name[:4]) for count, name in sorted(curr_scen[0], key=lambda x: x[1]))
                if "case_study" in args.scenario or "asym" in args.scenario:
                    scen_str = scen_str + "-".join("%i%s" % (count, name[:4]) for count, name in sorted(curr_scen[1], key=lambda x: x[1]))
            else:
...
def run_sequential(args, logger):
    ...
    if ('sc2custom' in args.env) or ('sc2drop' in args.env):
        rs = RandomState(0)
        args.env_args['scenario_dict'] = s_REGISTRY[args.scenario](rs=rs)
        args.scenario_len = len(args.env_args['scenario_dict']['scenarios'])
    ...
    if not args.entity_scheme:
        ...
        scheme = {
            ...
            "reward": {"vshape": (1,)},
            "terminated": {"vshape": (1,), "dtype": th.uint8},
            "scenario": {"vshape": (args.scenario_len,), "dtype": th.uint8}
        }
    ...
    while runner.t_env <= args.t_max:
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            test_returns_log = runner.test_returns_log
            learner.update_rew_log(test_returns_log)
            learner.update_bwm(runner.bwm)
        ...
        if args.save_model and (runner.t_env - model_save_time >= args.save_model_interval or
            ...
            bwm_save_path = os.path.join(args.local_results_path, "bwm", args.unique_token)
            bwm_name = '%s/bwm-%d.pkl' % (bwm_save_path, runner.t_env)
            os.makedirs(bwm_save_path, exist_ok=True)
            with open(bwm_name, 'wb') as f:
                pickle.dump(runner.bwm, f)
